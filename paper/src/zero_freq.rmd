# Effect of zero-freq words
Running regression models with only words that were in SUBTLEX-NL (i.e., has an estimated word frequency).
NB: The models are run with lme4 and not brms as the rest of the regression models.

Conclusion: When only including words in the SUBTLEX-NL corpus, we still find the same direction for the effect of frequency as found in the brms model reported in the paper.
...
## Load data
This chunk is copied from ``src/analysis.r``
```{r}
